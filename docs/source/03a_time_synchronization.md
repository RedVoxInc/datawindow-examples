...
now estimate the travel time, or latency, between the server and the device using these equations:

> Offset between server and device: (A2 - A1 - B2 + B1) / 2
>
> Offset between device and server: (B3 - B2 - A3 + A2) / 2

For a given DataWindow, we consider the lowest latency the best representative latency of the DataWindow.
...
than 0 (meaning the device time is ahead of the server's).  Whenever we update the timestamps, we create a new series 
of timestamps, in the event users are interested in the raw data timestamps.
